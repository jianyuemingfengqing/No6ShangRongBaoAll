...
export default {
  ...
  //nuxt服务器中 vue的cretead生命周期访问会在前端项目服务器中执行  而不是浏览器来执行，所以
  //要获取localStorage中的token时 代码如果是created执行会报错
  mounted() {
    ...
    showInfo() {
      ...
      if (token) {
        ...
        this.$axios({
          ...
          console.log(r.data.data.item);
          this.userInfo = r.data.data.item;
